...
<#
  ...
  - This script must be bootstrapped using a sysadmin account in order to create stored procedures, 
    a certificate and login, and a server role and on the target server. 
    Search this script's source for bootstrapCiDatabaseHandover to see the detail.
  ...
  A typical run will result in this:
  ...
  -----------------------------------------------------
    Will log in to Server=Z600.local as User dev to create:
    
      Database=dryRunDatabaseName 
      with
      database Owner=applicationdatabaseowner
      application Login=dryRunDatabaseName 
      application readonly login=dryRunDatabaseName_readonly 
      application login is dbowner= False
      Passwords generate by: This script
  -----------------------------------------------------
  ...
  Execute master.Ci.CreateDatabaseWithOwner
            @dbname  = 'dryRunDatabaseName',
            @dbowner = 'applicationdatabaseowner',
            @appLogin = 'dryRunDatabaseName',
            @appLoginPassword = 'mGXnxSAHTuBXA6GQGs0W',
            @appReadonlyLogin = 'dryRunDatabaseName_readonly',
            @appReadonlyLoginPassword = 'Pxslc1RISytfeblnMtLj',
            @appLoginIsDbOwner = 0
  ```
...
Param(
  ...
  [string][Parameter(ParameterSetName='Run')][ValidatePattern('(".+"|[^"]+)')]$databaseName,
  ...
  ##The password to use for -appReadonlyLogin. If none is given, one will be generated and displayed as the
  ...
  [string][Parameter(ParameterSetName='Run')]$databaseOwner,
  ...
  ##Use this switch, with a sysadmin login, to bootstrap the scripts needed on the server to use this script.
...
function sanitiseAndAutocompleteParameters{
  ...
  $script:appReadonlyLogin= [string]::IsNullOrWhiteSpace( $appReadonlyLogin ) ? (qadd $databaseName "_readonly") : $appReadonlyLogin.ToLower()
}
...
"
  ...
  runOrDryRun "Execute master.Ci.CreateDatabaseWithOwner
    ...
    if(-not $dryRun)
    {
      "Script finished. Verifying application login $appLogin can access database $dbName … "
      sqlcmd -S $serverInstance -d $databaseName -U $appLogin -P $appLoginPassword `
             -Q "Select 'Connected to database ' + Db_Name() + ' as ' + SYSTEM_USER + ' at ' + Cast(GetDate() as Varchar(20))"
    }
}

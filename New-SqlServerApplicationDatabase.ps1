...
<#
  .SYNOPSIS
  Create an MS SQL Server application database and roles for owner, application, and application_readonly
...
Param(
  ...
  ##on the server. Defaults to $env:SQLCMDUSER is set, or to blank if not, in which case Integrated Security is used.
  ...
  ##The password to use for -readonlyappLogin. If not is given, one will be generated and displayed as the
  ##second or first line of output of this script.
  [string]$readonlyappLoginPassword,
  
  ##Leave this blank to use the server default language
  [string]$loginLanguage,

  ## Leave this blank to use the server default collation
  ...
  ##If set, this Login will be made the owner of the newly created database. If blank, no owner Login is created and 
  ##the new database will remain owned by -ciLogin.
  ##- The Login must already exist, and -ciLogin must have Impersonate Permisson on it.
  ##- Creating the databaseOwner Login is a one-task per server. To create it, run this script once interactively
  ##  with -bootstrapSysAdminLogin='<a sysadmin login>'
...
function sanitiseAndAutocompleteParameters{
  if(-not (isValidTSqlIdentifier $databaseName)){
      throw "$databaseName is not a valid database name. Either surround it in `"Double Quotes`" or use a different name."
  }
...
"@
  ...
  if($dryRun){ "Bootstrap ApplicationDatabaseOwner and CILogin. 
...
You did not provide passwords, so passwords were created and shown above this line."
  ...
  $collationClause= ($databaseCollation) ? "With COLLATE $databaseCollation" : ""
